...
export function isInventoryUnit(value: string): value is InventoryUnit {
  return Object.hasOwn(unitDefinitions, value);
}
...
export function toBaseQuantity(
  ...
  return convertQuantity(
    quantity,
    unit,
    baseUnitByDimension[unitDefinitions[unit].dimension],
  );
}
...
export function formatQuantity(
  ...
  if (dimension === "mass" && quantity.abs().gte(1000)) {
    return `${quantity.div(1000).toDecimalPlaces(3).toString()} kg`;
  }
  if (dimension === "volume" && quantity.abs().gte(1000)) {
    return `${quantity.div(1000).toDecimalPlaces(3).toString()} L`;
  }

  return `${quantity.toDecimalPlaces(3).toString()} ${baseUnitByDimension[dimension]}`;
}

export function isLowStock(
  baseQuantity: Decimal.Value,
  threshold: Decimal.Value | null,
): boolean {
  return threshold !== null && new Decimal(baseQuantity).lte(threshold);
}
